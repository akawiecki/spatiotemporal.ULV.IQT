...
```{r fixed effect prediction data }
# sequence of days that is used to calculate the gaussian sigma=20 weighted spray effect
days <- days <- seq(0, 100, by = 1)

# calculate weighted spray effect using the gaussian sigma=20 decay function
g20.days <- as.data.frame(days) %>%
  mutate(g20_day.max = exp(-(days^2) / (2 * 20^2)))

# plot of weight distribution over days
plot.g20 <- ggplot(g20.days, aes(x = days, y = g20_day.max)) +
  geom_line() +
  theme_bw()
plot.g20
```
...
```{r fixed effect prediction stack}
# the stack for predictions without any random effects
# the response variable is set to NA so INLA can populate with the predicted value based on the model in the estimation stack
stack.pred.fixef <- inla.stack(
  data = list(y = NA),
  A = list(1, 1),
  effects = list(
    Intercept = rep(1, nrow(g20.days)),
    g20.weight = g20.days[["g20_day.max"]]
  ),
  tag = "pred.fixef"
)
```
...
```{r stack spatial est}
# the stack for estimation
stack.est <- inla.stack(
  data = list(y = data$AA_TOTAL),
  A = list(1, 1),
  effects = list(
    Intercept = rep(1, nrow(data)),
    g20.weight = data[["g20_day.max"]]
  ),
  tag = "est"
)
```
...
```{r formula}
# the model for prediction averages out the random effects (spatial, temporal, and household variations), that are set equal to their mean of zero.
# it uses only the fixed effect selected from the within-household model candidates to estimate Ae. aegypti density per household
formula <- y ~ -1 + Intercept + g20.weight
```


```{r m.g20.14}
# run INLA models for all stacks
m.g20.14 <- inla(formula,
...
saveRDS(m.g20.14, here("analysis", "data", "derived_data", "results", "m.g20.14.rds"))
```
...
```{r estimated number of ae aegypti}
# extract the indexes of stack full that correspond to the tag = "pred.fixef"
# that is, the prediction values
id.pred.fixef <- inla.stack.index(stack.all, "pred.fixef")$data

days <- days <- seq(0, 100, by = 1)

# summary.fitted.values: data frame with the mean, standard deviation, and quantiles of the fitted values obtained by transforming the linear predictors by the inverse of the link function
mg20.14.pred <- as.data.frame(days) %>%
  # weight value per day given a Gaussian function with sigma= 20
  mutate(
    g20_weight = exp(-(days^2) / (2 * 20^2)),
    # model prediction of the mean number of Ae. aegypti
    AA.pred = m.g20.14$summary.fitted.values[id.pred.fixef, "mean"],
    sd = m.g20.14$summary.fitted.values[id.pred.fixef, "sd"],
    # model prediction of the lower confidence interval of Ae. aegypti
    lower.ci = AA.pred - 2 * sd,
    # model prediction of the upper confidence interval of Ae. aegypti
    upper.ci = AA.pred + 2 * sd
  )



## a plot with the original data
ggplot(mg20.14.pred, aes(x = days, y = AA.pred)) +
  geom_ribbon(
    data = g20.days, aes(
      ymin = lower.ci,
      ymax = upper.ci
  ...
  geom_line(data = mg20.14.pred)
# geom_point()
```


```{r ae aegypti reduction}
# model parameter estimates
mg20.14.red.f.e <- round(exp(m.g20.14[["summary.fixed"]]), digits = 2)

# the intercept is the baseline average number of Ae. aegypti overall
baseline <- mg20.14.red.f.e$mean[[1]]
# in this case it's 0.55. in the original study it is 0.65


# To calculate percent reduction, you need to know the baseline value (before reduction) and the final value (after reduction). The formula to calculate percent reduction is:

# % Reduction = ((Baseline Value − Final Value)/Baseline Value) × 100%


mg20.14.red <- mg20.14.pred %>%
  filter(days <= 60) %>%
  # if 0.55 is 100% of expected Ae. aegypti when the spray effect = 0
  # what % of 0.55 is the predicted number of AA for each time point
  # and express in terms of reduction from 100%
  # ((baseline-AA.pred)/baseline)*100)
  mutate(
    reduction = ((baseline - AA.pred) / baseline) * 100,
    # because the lower.ci of AA.pred will be a higher value of reduction
    # it's actually the upper band
    u.red = ((baseline - lower.ci) / baseline) * 100,
    # viceversa for the upper.ci
    l.red = ((baseline - upper.ci) / baseline) * 100
  )

# what is the maximum reduction achieved by the spray effect?
# round(max(range(mg20.14.red$reduction)), 2)
# in the study it was 48.93564
...
interpolated_y <- linear_interpolation(25, mg20.14.red$reduction, mg20.14.red$days)

# 25.87502

# 25% reduction corresponds approximately to 25 days post spray
