...
gis.buff.13 <- readRDS(here("analysis", "data", "raw_data", "gis.buff.sample.13.rds"))

# extract the data frame of study households from the gis.buff.13 data frame that includes all households within 1000m buffer of every study household
data.13 <- gis.buff.13 %>%
  filter(!is.na(exp))
...
w.2013.bio <- fx.perd.bio(data.13, cores)


w.2013.bio <- w.2013.bio %>%
  left_join(vbles.time.13[, c("location_code","month", "date", "g20_day.max")] %>% st_drop_geometry(), by = c("location_code", "date"))

saveRDS(w.2013.bio, here("analysis", "data", "derived_data", "variables", "variables_2013", "vbles.space.ring.bio.13.rds"))
...
w.2013.100m <- fx.perd.100m(data.13, cores)


w.2013.100m <- w.2013.100m %>%
  left_join(vbles.time.13[, c("location_code","month", "date", "g20_day.max")] %>% st_drop_geometry(), by = c("location_code", "date"))


saveRDS(w.2013.100m, here("analysis", "data", "derived_data", "variables", "variables_2013", "vbles.space.ring.100m.13.rds"))
