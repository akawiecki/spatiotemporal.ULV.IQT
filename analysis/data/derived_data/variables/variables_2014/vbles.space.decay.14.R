...
fx.w <- function (df, ncores) {
  ...
  fx.dist.w <- function (x) {
    ...
    df.w <- df.w %>% 
      ...
      select(c("i_total", "g5_total", "g25_total", 
...
w.2014 <- fx.w(vbles.time.14,cores)

saveRDS(w.2014 , here("analysis", "data", "derived_data", "variables","variables_2014", "vbles.space.decay.14.rds"))
