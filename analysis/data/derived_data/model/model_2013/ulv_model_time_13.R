  ...
  fx.model <- function(df, vble.list) {
    stack <- inla.stack(
      ...
      effects = list(
        c(sp.index, list(Intercept = 1)),
        list(
          # vble = one variable from a list of candidate variables that measure the effect of sprays within household i
          vble = df[[vble.list]],
...
saveRDS(m.time.list, here("analysis", "data", "derived_data", "model", "model_2013", "model_outputs_2013", "m.time.13.rds"))
